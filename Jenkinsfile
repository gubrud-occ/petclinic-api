  ...
  stages {
   ...
   stage('Get API Testing Repo')
   {
      steps {
        ...
        git(
        url:'git@github.com:gubrud-occ/petclinic-api.git',
        credentialsId: 'api',
        branch: "main"
        )
     }
        ...
        steps
        {
           ...
           echo "ls -l"
              script {
                 ...
                 FindInstancePublicIP =  "aws --region us-east-2  ec2 describe-instances --filters \\\'Name=tag:build_id,Values=\\\"TAG_TO_REPLACE\\\"\\\' | grep -i PublicIpAddress | awk '{print \$2 }' | awk '{print substr(\$1,2); }' | awk '{print substr(\$1, 1, length(\$1)-2)}'";
